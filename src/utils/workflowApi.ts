...
export async function loadWorkflowFromApi(
  ...
  try {
    const response = await fetch(`http://localhost:8787/api/workflow/${templateId}`, {
      method: "GET",
      headers: {
    ...
    const data = (await response.json()) as WorkflowApiResponse | WorkflowApiError;
    ...
    console.error(`[Frontend API] Failed to load workflow ${templateId} from API:`, error);
    throw error;
  }
}
...
export async function loadWorkflowSafe(
  ...
  try {
    ...
    console.warn(`[Frontend API] API load failed for ${templateId}, falling back to local files:`, apiError);
    ...
    try {
      ...
      console.error(`[Frontend API] Both API and local loading failed for ${templateId}:`, {
        apiError,
        localError,
      });
      throw new Error(
...
export async function checkApiAvailability(): Promise<boolean> {
  try {
    const response = await fetch("http://localhost:8787/api/workflow/health", {
      method: "GET",
