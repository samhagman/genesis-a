...
test.describe("Real-Time Workflow Updates", () => {
  ...
  test("should create new workflow via chat", async ({ page }) => {
    ...
    await chatInput.fill('Create a new workflow called "Real-Time Test Workflow"');
    ...
    const hasWorkflowText = pageContent.includes('workflow') || pageContent.includes('Workflow');
  ...
  test("should test WebSocket connection setup", async ({ page }) => {
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (text.includes("WebSocket") || text.includes("[ChatPanel]") || text.includes("🔌")) {
  ...
  test("should add goal and test real-time updates", async ({ page }) => {
    ...
    // Add a goal via chat
    await chatInput.fill('Add a goal called "Real-Time Test Goal" to verify live updates');
    await sendButton.click();
    ...
    const hasNewGoal = updatedContent.includes('Real-Time Test Goal') || 
    ...
    const goalElements = await page.locator('[class*="goal"], [class*="card"], h3, h4').count();
  ...
  test("should verify error handling", async ({ page }) => {
    ...
    await chatInput.fill('Do something completely invalid that should not work');
    await sendButton.click();
