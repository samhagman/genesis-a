...
function checkEnvVar(varName, description) {
  ...
  if (fs.existsSync(devVarsPath)) {
    ...
    const isNotPlaceholder = !content.includes(varName + '=your-') && !content.includes(varName + '=sk-your-');
...
function main() {
  ...
  allValid &= checkFile(path.join(projectRoot, 'wrangler.jsonc'), 'wrangler.jsonc file');
  allValid &= checkFile(path.join(projectRoot, 'tests/integration-cost-money.test.ts'), 'Expensive tests file');
  ...
  if (allValid) {
    ...
    console.log('');
    console.log('See tests/EXPENSIVE_TESTS.md for detailed setup instructions.');
    process.exit(1);
  }
}
